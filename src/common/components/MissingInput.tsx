...
export const MissingInput: FunctionComponent<MissingInputProps> = ({ name, roomId, timeLimit, categoryList, playerList }) => {
  ...
  if (!isUndefined(playerList) && playerList.length === 0) {
    reminders.push(<div key={5}>At least one player must join the game before you can start.</div>);
  }
